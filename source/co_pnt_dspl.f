! Compute displaced positions of the shock/discontinuity points
      ! module RR_modify
      !   implicit none
      !   integer boundary_code
      ! end module RR_modify
      subroutine co_pnt_dspl(xysh,
     +                       xyshu,
     +                       xyshd,
     +                       zroeshu,
     +                       nodcodsh,
...
!     .. local scalars ..
      double precision dx,dy,dum,tx,ty,xi,yi
      ...
      integer seg_1, seg_2
      double precision dx_1, dy_1, dx_2, dy_2 !unit vectors 
      integer ibndfac(3,*)
      !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
      !intersection variables
      double precision xi1, yi1, xi2, yi2 !incident
      double precision xr1, yr1, xr2, yr2 !reflected
      double precision xin, yin, xr, yr !intersection points
      double precision XIU(2), YIU(2), XID(2), YID(2),XRU(2),YRU(2), XRD(2), YRD(2) !for upstream and downstream shock points
      integer n
      double precision L, dx_n, dy_n,slope
      !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
      do ish = 1,nshmax
        do i = 1,npshmax
          nodcodsh(i,ish)=-99
...
!       write(*,*)shinspps(1,1,isppnts),shinspps(2,1,isppnts)
     ...
     +     typespecpoints(isppnts).eq.'WDGX')then!vivek
     ...
     +         typespecpoints(isppnts).eq.'WPNRY'.or.
     +         typespecpoints(isppnts).eq.'WDGY')then!vivek
    !  +         typespecpoints(isppnts).eq.'PTY')then !vivek
...
!         find interpolation point
          ...
          ! PRINT*, xi,yi
          open(2,file='log/points_check.log')
          write(2,*)'downstream of incident'
          write(2,*) xc(1), yc(1), xc(2), yc(2)
          write(2,*)'upstream of reflected'
          write(2,*) xs(1), ys(1), xs(2), ys(2)
          write(2,*) 'intersection point'
          write(2,*) xi, yi
          close(2)
