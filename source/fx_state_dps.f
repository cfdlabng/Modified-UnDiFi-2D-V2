...
!     .. scalar arguments ..
     ...
     +        nspecpoints,shinspps(2,5,*),ispclr(5,*) !vivek
      character*1 typesh(*)
...
!       determine shock and index of the extreme
     ...
     +   typespecpoints(isppnts).eq.'IPY')then
      ...
      ! restore previous upstream and downstream state !vivek
        do k=1, ndof
...
! find boundary segment where the shock point moves
           clr=5
           DO clr=1,NCLR
            IF(ICLR(clr).eq.ispclr(1,ISPPNTS))exit !vivek changed the array size of ispclr to 2D 
           enddo
...
! Vale
         WRR=xtp(15)
! Vale
         open(2,file='log/specialpointRR_check.log')
...
! propagate to the other shock points the new velocity
         ...
         wsh(2,ip2,ish2)= wsh(2,ip1,ish1)
         write(2,*) 'incident'
         write(2,*) wsh(1,ip1,ish1), wsh(2,ip1,ish1)
         write(2,*) 'reflected'
         write(2,*) wsh(1,ip2,ish2), wsh(2,ip2,ish2)
         close(2)
